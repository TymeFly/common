...
public class CopyTest {
    ...
    public void test_Copy() {
        Document actual = source.accept(new Copy());

        Assert.assertNotSame("Same instance returned", source, actual);
        Assert.assertEquals("Unexpected Copy", source, actual);
        Assert.assertTrue("Failed to copy EMPTY_DOC", actual.contains(Key.EMPTY_DOC));
        Assert.assertTrue("Failed to copy EMPTY_ARRAY", actual.contains(Key.EMPTY_ARRAY));
        Assert.assertEquals("Unexpected Wrappers", Collections.emptyList(), actual.wraps());
    }
}