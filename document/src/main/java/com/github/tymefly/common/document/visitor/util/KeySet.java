...
/**
 * A visitor that calculates the keys that are stored in a Document.
 * @see PathSet
 * @see EntrySet
 */
@NotThreadSafe
public non-sealed class KeySet extends AbstractKeyVisitor<Set<DocumentKey>> {
    private final Set<DocumentKey> results = new LinkedHashSet<>();


    /**
     * A constructor for an {@link KeySet} visitor which will return all the keys in the Document.
    ...
    /**
     * A customised version of the {@link KeySet} Visitor
     * @param option        One of the options that determine which keys to retrieve
     */
    public KeySet(@Nonnull VisitorOptions option) {
     ...
     */
    public KeySet(@Nonnull VisitorOptions option, VisitorOptions... options) {
     ...
     */
    public KeySet(@Nonnull EnumSet<VisitorOptions> options) {
        super(options);
    }
    ...
    @Nonnull
    @Override
    public Set<DocumentKey> process() {
        return results;
    }
}
